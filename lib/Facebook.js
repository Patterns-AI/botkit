...
function Facebookbot(configuration) {
    ...
    facebook_botkit.defineBot(function(botkit, config) {
        ...
        bot.send = function(message, cb) {
            ...
            message.access_token = configuration.access_token_cb(message.page);
            delete message['page'];
            request({
                method: 'POST',
                json: true,
    ...
    var messenger_profile_api = {
        greeting: function(payload,page) {
            var message = {
                'greeting': []
            };
            ...
            facebook_botkit.api.messenger_profile.postAPI(message,page);
        },
        delete_greeting: function(page) {
            facebook_botkit.api.messenger_profile.deleteAPI('greeting',page);
        },
        get_greeting: function(page, cb) {
            facebook_botkit.api.messenger_profile.getAPI('greeting', cb,page);
        },
        get_started: function(payload,page) {
            var message = {
                'get_started': {
                    'payload': payload
                }
        ...
        delete_get_started: function(page) {
            facebook_botkit.api.messenger_profile.deleteAPI('get_started',page);
        },
        get_get_started: function(page, cb) {
            facebook_botkit.api.messenger_profile.getAPI('get_started', cb,page);
        },
        menu: function(payload, page) {
            var messege = {
                persistent_menu: payload
            };
            facebook_botkit.api.messenger_profile.postAPI(messege, page);
        },
        delete_menu: function(page) {
            facebook_botkit.api.messenger_profile.deleteAPI('persistent_menu', page);
        },
        get_menu: function(page, cb) {
            facebook_botkit.api.messenger_profile.getAPI('persistent_menu', page, cb);
        },
        account_linking: function(payload, page) {
            var message = {
                'account_linking_url': payload
            };
            facebook_botkit.api.messenger_profile.postAPI(message, page);
        },
        delete_account_linking: function(page) {
            facebook_botkit.api.messenger_profile.deleteAPI('account_linking_url', page);
        },
        get_account_linking: function(page, cb) {
            facebook_botkit.api.messenger_profile.getAPI('account_linking_url', page, cb);
        },
        domain_whitelist: function(payload, page) {
            var message = {
                'whitelisted_domains': Array.isArray(payload) ? payload : [payload]
            };
            facebook_botkit.api.messenger_profile.postAPI(message, page);
        },
        delete_domain_whitelist: function(page) {
            facebook_botkit.api.messenger_profile.deleteAPI('whitelisted_domains', page);
        },
        get_domain_whitelist: function(page, cb) {
            facebook_botkit.api.messenger_profile.getAPI('whitelisted_domains', page, cb);
        },
        target_audience: function(payload, page) {
            var message = {
                'target_audience': payload
            };
            facebook_botkit.api.messenger_profile.postAPI(message, page);
        },
        delete_target_audience: function(page) {
            facebook_botkit.api.messenger_profile.deleteAPI('target_audience', page);
        },
        get_target_audience: function(page, cb) {
            facebook_botkit.api.messenger_profile.getAPI('target_audience', page, cb);
        },
        home_url: function(payload, page) {
            var message = {
                home_url: payload
            };
            facebook_botkit.api.messenger_profile.postAPI(message, page);
        },
        delete_home_url: function(page) {
            facebook_botkit.api.messenger_profile.deleteAPI('home_url', page);
        },
        get_home_url: function(page, cb) {
            facebook_botkit.api.messenger_profile.getAPI('home_url', page, cb);
        },
        postAPI: function(message, page) {
            let access_token = page ? configuration.access_token(page) : configuration.access_token;
            request.post('https://' + api_host + '/v2.6/me/messenger_profile?access_token=' + access_token,
                {form: message},
                function(err, res, body) {
                    if (err) {
        ...
        deleteAPI: function(type, page) {
            let access_token = page ? configuration.access_token(page) : configuration.access_token;
            var message = {
                'fields': [type]
            };
            request.delete('https://' + api_host + '/v2.6/me/messenger_profile?access_token=' + access_token,
                {form: message},
                function(err, res, body) {
                    if (err) {
                        facebook_botkit.log('Could not configure messenger profile');
        ...
        getAPI: function(fields, page, cb) {
            let access_token = page ? configuration.access_token(page) : configuration.access_token;
            request.get('https://' + api_host + '/v2.6/me/messenger_profile?fields=' + fields + '&access_token=' + access_token,
                function(err, res, body) {
                    if (err) {
        ...
        get_messenger_code: function(image_size, cb, ref, page) {
            var message = {
                'type': 'standard',
            ...
            request.post('https://' + api_host + '/v2.6/me/messenger_codes?access_token=' + configuration.access_token_cb(page),
    ...
    var attachment_upload_api = {
        upload: function(attachment, page, cb) {
            var message = {
                message: {
            ...
            request.post('https://' + api_host + '/v2.6/me/message_attachments?access_token=' + configuration.access_token_cb(page),
                { form: message },
                function(err, res, body) {
                    if (err) {
    ...
    var tags = {
        get_all: function(page, cb) {
            request.get('https://' + api_host + '/v2.6/page_message_tags?access_token=' + configuration.access_token_cb(page),
                function(err, res, body) {
                    if (err) {
    ...
    var nlp = {
        enable: function(custom_token, page) {
            facebook_botkit.api.nlp.postAPI(true, custom_token, page);
        },
        disable: function(custom_token, page) {
            facebook_botkit.api.nlp.postAPI(false, custom_token, page);
        },
        postAPI: function(value, custom_token, page) {
            let access_token = page ? configuration.access_token(page) : configuration.access_token;
            var uri = 'https://' + api_host + '/v2.8/me/nlp_configs?nlp_enabled=' + value + '&access_token=' + access_token;
            if (custom_token) {
    ...
    var user_profile = function(uid, fields, page, cb) {
        if (!fields) {
            fields = 'first_name,last_name,timezone,gender,locale';
        }
        return new Promise(function(resolve, reject) {
            var uri = 'https://' + api_host + '/v2.6/' + uid + '?fields=' + fields + '&access_token=' + configuration.access_token_cb(page);
            request.get(uri, {},
                function(err, res, body) {
                    if (err) {
